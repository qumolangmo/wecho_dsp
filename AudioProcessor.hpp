...
private:
    std::unique_ptr<CrossFader<BassEffect>> EBass;
    std::unique_ptr<CrossFader<ClarityEffect>> EClarity;
    std::unique_ptr<GainEffect> EGain;
    std::unique_ptr<ChannelBalanceEffect> EChannelBalance;
    std::unique_ptr<CrossFader<EvenHarmonicEffect>> EEvenHarmonic;
    std::unique_ptr<CompressorEffect> ECompressor;
    std::unique_ptr<CrossFader<ConvolveEffect>> EConvolve;
    std::unique_ptr<CrossFader<VirtualBassEffect>> EVirtualBass;
    std::unique_ptr<CrossFader<LookAheadSoftLimitEffect>> ELookAheadSoftLimiter;
    std::unique_ptr<CrossFader<LowCatEffect>> ELowCat;
    std::unique_ptr<CrossFader<IIREqualizerEffect>> EIIREQualizer;
    std::unique_ptr<CrossFader<ReverbEffect>> EReverb;
...
private:
    AudioProcessor()
        : audio_stream()
        , EBass(std::make_unique<CrossFader<BassEffect>>(50.0, false, 0, 1.48f, 60.0f))
        , EClarity(std::make_unique<CrossFader<ClarityEffect>>(50.0, false, 0))
        , EGain(std::make_unique<GainEffect>(false, 0))
        , EChannelBalance(std::make_unique<ChannelBalanceEffect>(false, 0))
        , EEvenHarmonic(std::make_unique<CrossFader<EvenHarmonicEffect>>(80.0, false, 0.0f, 0.0f, 0.0f))
        , EConvolve(std::make_unique<CrossFader<ConvolveEffect>>(100, false, 0.1f))
        , ECompressor(std::make_unique<CompressorEffect>(false))
        , ELookAheadSoftLimiter(std::make_unique<CrossFader<LookAheadSoftLimitEffect>>(100, false))
        , ELowCat(std::make_unique<CrossFader<LowCatEffect>>(30, false, 120))
        , EIIREQualizer(std::make_unique<CrossFader<IIREqualizerEffect>>(30, false))
